...
module.exports = ConfigEtcd = (function(_super) {
  ...
  ConfigEtcd.prototype._loadBaseConfig = function() {
    ...
    env = argv.VINSIGHT_ENV || process.env.VINSIGHT_ENV;
    ...
    this.baseConfig = _.cloneDeep(JSON.parse(config.toString()), function(value) {
      ...
      if (value.indexOf("ENV::") === 0) {
        return process.env[value.substring(5)] || ("ENV_VAR_" + envVar + "_NO_SET");
      }
    });
    ...
    for (key in _ref) {
      ...
      _results.push((function(_this) {
        return function(key, value) {
          var service;
          service = value.substring(11);
          if (!(value.indexOf('ETCD_') === 0 || (_this.services[service] != null))) {
            if (--remaining === 0) {
              cb();
            }
            return;
